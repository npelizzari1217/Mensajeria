...
El sistema MUST permitir a Admin asociar un usuario existente a una empresa creando el vínculo `UserEmpresa`. El endpoint MUST requerir AuthGuard, RolesGuard y `@Roles(Admin)`.

#### Scenario: Asignar usuario existente
- GIVEN un Admin autenticado, una empresa existente y un usuario existente
- WHEN POST `/empresas/:id/users`
- THEN 201 y se crea el registro `UserEmpresa`
