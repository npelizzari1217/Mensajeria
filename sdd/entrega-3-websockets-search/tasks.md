...
- [x] 1.3 Create migration: add `fts tsvector` generated column + GIN index on `messages`
...
- [x] 2.2 Implement `search()` in `PrismaMessageRepository` using `$queryRaw` with `to_tsvector`/`plainto_tsquery`
...
- [x] 3.2 Create `ws-auth.middleware.ts` — extract JWT from handshake, verify, attach userId to socket
- [x] 3.3 Create `MessagingGateway` — namespace `/messages`, auth middleware, rooms by `user:{userId}`
- [x] 3.4 Create `WebSocketModule` — exports `MessagingGateway`
- [x] 3.5 Create `WebSocketHandler` — implements EventHandler, bridges `MessageSent`/`MessageRead` to gateway emits
- [x] 3.6 Wire `WebSocketHandler` subscription in `EventBusModule.onModuleInit()`
- [x] 3.7 Import `WebSocketModule` in `AppModule`, provide `SearchMessagesUseCase` in `MessagingModule`

## Phase 4: Web Integration

- [x] 4.1 Add `socket.io-client` to `web/package.json`
- [x] 4.2 Create `SocketContext` — connects to `/messages` with JWT auth, exposes `onMessageNew` callback
- [x] 4.3 Modify `InboxPage` — register `message:new` listener via `SocketContext`, re-fetch inbox on event
- [x] 4.4 Create `SearchPage` — search input with debounce, GET `/messages/search`, results table
- [x] 4.5 Modify `App.tsx` — wrap with `SocketProvider`, add `/search` route
...
- [x] 5.4 Unit test: `MessagingGateway` auth — JWT handshake grants access, invalid JWT rejected
- [x] 5.5 Unit test: `WebSocketHandler` — mock gateway, emit domain events, verify gateway emit calls
